...
def generate_summaries(item, sourcedirectory, llamakey, gptkey, vector_size, max_attempts, max_tokens, only_general_summary = False):
    ...
    for i in meeting_participants:
        success, questions_extract = failsafe(get_questions, max_attempts, participant=i, transcript=transcript, gptkey=gptkey, max_tokens=4000)
    ...
    for i in total_questions:
        ...
        success, answer = answer_questions(i['questions'], transcript, supplementary, vector_size, gptkey, max_attempts,  max_tokens)
        if not success:
            overallsuccess = False
        i['questions'] = answer
    ...
    for i in total_questions:
        ...
        gptentry['summary'] = cwm.askgpt("Summarize this transcript. Create an abstractive summary. Make the summary 250 tokens or less." + modified_transcript, gptkey, 250)
...
def get_questions(participant, transcript, gptkey, max_tokens):
...
def answer_questions(questions, transcript, supplementary, vector_size, gptkey, max_attempts, max_tokens):
    supplementary_data = []
    ...
    for i in quevectors:
        success, i['answer'] = failsafe(get_gpt_json, max_attempts, quevector=i, transcript=transcript, gptkey=gptkey, max_tokens=max_tokens)
...
if __name__ == '__main__':
    ...
    sumtargetdirectory = 'code/sum_output'  # where to save to after the summary step
    evaltargetdirectory = 'code/eval_output'  # where to save to after the evaluation step
    time_start = time.time()
    runthrough("ES2002c.json", sourcedirectory, sumtargetdirectory, evaltargetdirectory, llamakey, gptkey, max_tokens, max_attempts, vector_size, only_general_summary = True)
    time_end = time.time()
