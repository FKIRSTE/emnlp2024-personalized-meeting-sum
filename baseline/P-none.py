...
def build_prompt(transcript, target):
    role = (
...
def apply_functions_to_row(row, model):
    ...
    prompt1 = build_prompt(row['transcript'], 'Industrial Designer')
    prompt2 = build_prompt(row['transcript'], 'User Experience')
    prompt3 = build_prompt(row['transcript'], 'Marketing')
    prompt4 = build_prompt(row['transcript'], 'Project Manager')
    
    # Process each prompt using the model's safe_model_call method
    predicted_summary_p1 = model.safe_model_call(prompt1, 1000)
    print(predicted_summary_p1, flush=True)

    predicted_summary_p2 = model.safe_model_call(prompt2, 1000)
    print(predicted_summary_p2, flush=True)

    predicted_summary_p3 = model.safe_model_call(prompt3, 1000)
    print(predicted_summary_p3, flush=True)

    predicted_summary_p4 = model.safe_model_call(prompt4, 1000)    
...
df.to_csv(f'{DIR}result/ami_gpt_target_noAcc.csv', index=False)